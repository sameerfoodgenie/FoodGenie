...
const PLAN_COMPARISON = [
  { id: 'starter', name: 'Starter', price: '₹99', tokens: '100', color: '#4ADE80' },
  { id: 'smart_foodie', name: 'Smart Foodie', price: '₹199', tokens: '300', color: '#818CF8' },
  { id: 'genie_pro', name: 'Genie Pro', price: '₹499', tokens: '1000', color: '#D4AF37', best: true },
];
      ...
      <Pressable style={s.overlay} onPress={handleDismiss}>
        <Pressable style={[s.modal, { backgroundColor: colors.surface }]} onPress={e => e.stopPropagation()}>
          ...
          <Animated.View entering={FadeIn.duration(400)} style={s.headerWrap}>
            <LinearGradient
              colors={isExpired ? ['#FF6B6B', '#FF8E53'] : ['#FFB347', '#FF8E53']}
              style={s.headerIcon}
            >
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(350)} style={s.loseSection}>
            ...
            <View style={s.loseList}>
              {FEATURES_LOST.map((feat, i) => (
                <View key={i} style={[s.loseItem, { backgroundColor: isDark ? 'rgba(255,107,107,0.06)' : 'rgba(255,107,107,0.04)' }]}>
                  <Text style={{ fontSize: 16 }}>{feat.emoji}</Text>
                  <Text style={[s.loseItemText, { color: colors.textSecondary }]}>{feat.label}</Text>
                  <MaterialIcons name="block" size={14} color="rgba(255,107,107,0.6)" />
                </View>
              ))}
            </View>
          </Animated.View>
