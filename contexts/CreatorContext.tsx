...
export const CREATOR_LEVELS: CreatorLevelInfo[] = [
  ...
  { id: 'rising_creator', name: 'Rising Creator', emoji: '🚀', color: '#D4AF37', minPosts: 10 },
  { id: 'food_influencer', name: 'Food Influencer', emoji: '⭐', color: '#FFD700', minPosts: 25 },
  { id: 'genie_creator', name: 'Genie Creator', emoji: '🧞', color: '#FFC107', minPosts: 50 },
  { id: 'elite_chef', name: 'Elite Chef', emoji: '👨‍🍳', color: '#B8960C', minPosts: 100 },
];
...
export function CreatorProvider({ children }: { children: ReactNode }) {
  ...
  const milestones: Milestone[] = useMemo(() => [
    ...
    { id: 'act_10', category: 'activity' as MilestoneCategory, title: 'Getting Serious', description: 'Post 10 meals', icon: '🔥', target: 10, current: Math.min(postCount, 10), isUnlocked: postCount >= 10, reward: 'Rising Creator Badge', color: '#D4AF37' },
    { id: 'act_25', category: 'activity' as MilestoneCategory, title: 'Feed Star', description: 'Post 25 meals', icon: '⚡', target: 25, current: Math.min(postCount, 25), isUnlocked: postCount >= 25, reward: 'Feed Boost', color: '#FFD700' },
    { id: 'act_50', category: 'activity' as MilestoneCategory, title: 'Powerhouse', description: 'Post 50 meals', icon: '💎', target: 50, current: Math.min(postCount, 50), isUnlocked: postCount >= 50, reward: 'Profile Highlight', color: '#FFC107' },
    // Consistency
    { id: 'str_7', category: 'consistency' as MilestoneCategory, title: 'Week Warrior', description: '7-day streak', icon: '🗓', target: 7, current: Math.min(streakCount, 7), isUnlocked: streakCount >= 7, reward: 'Streak Badge', color: '#D4AF37' },
    { id: 'str_15', category: 'consistency' as MilestoneCategory, title: 'Consistent Chef', description: '15-day streak', icon: '🏆', target: 15, current: Math.min(streakCount, 15), isUnlocked: streakCount >= 15, reward: 'Gold Streak Frame', color: '#FFD700' },
    { id: 'str_30', category: 'consistency' as MilestoneCategory, title: 'Iron Discipline', description: '30-day streak', icon: '👑', target: 30, current: Math.min(streakCount, 30), isUnlocked: streakCount >= 30, reward: 'Exclusive Avatar Ring', color: '#B8960C' },
    // Impact
    { id: 'imp_100', category: 'impact' as MilestoneCategory, title: 'Crowd Favorite', description: 'Get 100 likes', icon: '❤️', target: 100, current: Math.min(totalLikes, 100), isUnlocked: totalLikes >= 100, reward: 'Visibility Boost', color: '#D4AF37' },
    { id: 'imp_500', category: 'impact' as MilestoneCategory, title: 'Trending Creator', description: 'Get 500 likes', icon: '🌟', target: 500, current: Math.min(totalLikes, 500), isUnlocked: totalLikes >= 500, reward: 'Featured Creator', color: '#FFD700' },
    { id: 'imp_1000', category: 'impact' as MilestoneCategory, title: 'Food Legend', description: 'Get 1000 likes', icon: '🎯', target: 1000, current: Math.min(totalLikes, 1000), isUnlocked: totalLikes >= 1000, reward: 'Monetization Unlock', color: '#FFC107' },
  ...
  const badges: Badge[] = useMemo(() => [
    ...
    { id: 'b_rising', name: 'Rising Star', emoji: '🚀', description: 'Posted 10 meals', isUnlocked: postCount >= 10, color: '#D4AF37' },
    { id: 'b_influencer', name: 'Influencer', emoji: '⭐', description: 'Posted 25 meals', isUnlocked: postCount >= 25, color: '#FFD700' },
    { id: 'b_streak7', name: 'Streak Warrior', emoji: '🔥', description: '7-day streak', isUnlocked: streakCount >= 7, color: '#D4AF37' },
    { id: 'b_streak30', name: 'Iron Chef', emoji: '👑', description: '30-day streak', isUnlocked: streakCount >= 30, color: '#B8960C' },
    { id: 'b_liked', name: 'Fan Favorite', emoji: '❤️', description: '100 likes earned', isUnlocked: totalLikes >= 100, color: '#D4AF37' },
    { id: 'b_legend', name: 'Food Legend', emoji: '🎯', description: '1000 likes earned', isUnlocked: totalLikes >= 1000, color: '#FFD700' },
    { id: 'b_showmaker', name: 'Showmaker', emoji: '🎬', description: 'Created a show', isUnlocked: shows.length > 0, color: '#D4AF37' },
